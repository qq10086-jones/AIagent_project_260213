...
export const STEP_CONTRACTS = {
  pm_spec: {
    ...
    instructions: [
      ...
      "READ-ONLY DETAIL VIEW (mandatory): Every entity with an edit form MUST have a SEPARATE read-only detail view. The detail view displays all fields (including read-only metadata like created_at, updated_at, created_by) in label-value pairs, with explicit [Edit] and [Delete] buttons that transition to edit form / delete confirmation. The edit form pre-populated with data is NOT the detail view — users need a viewing experience distinct from editing. Write a user story: 'As a user I want to VIEW entity details without risking accidental edits'. acceptance.json MUST have a criterion per entity for detail view.",
      "ACTIVITY FEED (conditional): If the Goal includes 'activity', 'recent actions', 'audit trail', 'history', or a dashboard showing what changed over time, the spec MUST require a UNIFIED CHRONOLOGICAL activity feed. This is NOT two separate 'Recent X' lists side by side. It is one stream of entries like 'John created ticket T-42 at 10:04' + 'Lisa closed customer C-7 at 10:15', sorted descending by timestamp. Requires a backend activity_log table (entity_type, entity_id, action, actor, timestamp, summary). acceptance.json MUST include an activity-feed criterion checking for verbs like 'created'/'updated'/'deleted' in feed output.",
      "DASHBOARD QUICK ACTIONS (conditional): If the Goal describes a dashboard as the app landing page, the spec MUST require a 'Quick Actions' row at the top of dashboard with prominent buttons (≥2) for each module's primary create action: '+ Add Customer', '+ New Ticket' etc. These are top-level shortcuts, not buttons buried inside per-module cards. A user landing on the dashboard can initiate the most common tasks with one click.",
    ],
  },
  arch_design: {
    ...
    instructions: [
      ...
      "Under each plan/interfaces.md heading, include request shape, response shape or payload shape, auth requirement, AND error responses (400 validation with {error, field, message}; 404 for missing resources; 500 for server errors).",
      "VALIDATION CONTRACT (mandatory): Derive validation rules from plan/spec.md — do NOT invent CRM-specific enums. For each POST/PUT, document in interfaces.md: required fields (from spec's non-optional fields); enum whitelists (from spec's documented allowed values for any field); max-length (from spec or default 500 chars); foreign-key existence checks (from spec's relationships). If the spec does not constrain a field, omit that rule rather than making one up.",
      ...
      "READ-ONLY DETAIL ENDPOINT: For every entity, ensure GET /api/<resource>/:id returns the FULL record (including metadata like created_at/updated_at) to power the FE's read-only detail view. List this endpoint explicitly in interfaces.md even if it seems obvious.",
      "ACTIVITY FEED (conditional): If spec.md requires an activity feed, arch MUST add: (a) backend activity_log table schema (id, entity_type, entity_id, action, actor, timestamp, summary); (b) GET /api/activity endpoint returning latest N entries; (c) hook points in POST/PUT/DELETE handlers of other entities to insert into activity_log. workplan.json be_tasks MUST include an 'activity_log table + GET /api/activity' task.",
    ],
  },
  impl_fe: {
  ...
  impl_fe_skeleton: {
    ...
    instructions: [
      ...
      "Build app.js with: (a) showToast(message, type) function (escape message before innerHTML), (b) showConfirmDialog(message, onConfirm) function — the message MUST be wrapped in escapeHtml() before innerHTML assignment (critical XSS), (c) renderLoading(container) function, (d) renderEmptyState(container, message, actionLabel, onAction) function, (e) escapeHtml(text) function that escapes &, <, >, \", ', (f) apiFetch(url, options) wrapper with error handling, (g) client-side routing via switchView() that calls render functions for each module, (h) setupNavigation() to wire nav clicks, (i) STUB render function for each module: renderDashboardView(), renderCustomersView(), renderTicketsView(), etc. — each stub should call renderLoading() then show a placeholder.",
      "Use same-origin relative API paths only. Do not hardcode localhost URLs.",
  ...
  impl_fe_modules: {
    ...
    instructions: [
      ...
      "For EACH module, replace the stub render function with a full implementation: (a) list view with table, empty state when no data, loading indicator during fetch; (b) create form in a modal with required field validation and inline errors; (c) edit form pre-populated with existing data; (d) delete with showConfirmDialog confirmation; (e) showToast after every successful create/update/delete; (f) error handling with showToast on failure; (g) READ-ONLY detail view (separate modal or panel) that fetches GET /api/<resource>/:id, renders every field in <dl>/<dd> label-value format including created_at/updated_at, with [Edit] and [Delete] buttons — NOT the edit form pre-populated. The row's 'View' button opens this, the 'Edit' button opens the edit form.",
      "DASHBOARD QUICK ACTIONS (conditional): If the spec requires quick actions on the dashboard, renderDashboardView() MUST render a top-row of prominent buttons (class='btn btn-primary btn-lg') for each module's primary create action (e.g. '+ Add Customer', '+ New Ticket'), BEFORE the stat cards. Each button calls the corresponding showXForm() to open the create modal. These are distinct from buttons inside per-module cards.",
      "ACTIVITY FEED (conditional): If the spec requires an activity feed, renderDashboardView() MUST fetch GET /api/activity and render a single chronological list: each entry shows the summary text (e.g. 'Customer Acme was created'), a relative timestamp ('5 minutes ago'), and an action icon. NOT two separate 'Recent Customers' + 'Recent Tickets' tables. Single stream, newest-first.",
      "SECURITY (mandatory): Treat every field that came from the API as user-controlled unless it is a literal fetched by key lookup. Wrap such values with escapeHtml() before injecting into innerHTML/outerHTML/insertAdjacentHTML/template literals. Covers both visible text AND attribute contexts. Bad: `Delete ${customer.name}?`. Good: `Delete ${escapeHtml(customer.name)}?`. For class attributes whose value varies with user data (e.g. status badges), use a static whitelist map (const CLASSES = {allowed: 'badge-ok', ...}; const cls = CLASSES[value] || 'badge-default';), NEVER raw interpolation (`class=\"badge ${status.toLowerCase()}\"`).",
      "ASYNC SAFETY: If an async loader (e.g. loadCustomersForSelect) populates DOM inside a modal, capture a reference to the current modal and check it's still mounted before appendChild, to avoid errors when user closes modal mid-fetch.",
  ...
  impl_be: {
    ...
    instructions: [
      ...
      "INPUT VALIDATION (mandatory): For each POST/PUT handler, derive validation rules from plan/spec.md and plan/interfaces.md — NOT from canned examples. Reject (a) missing/blank fields the spec marks required, with 400 {error, field, message}; (b) enum values outside the whitelist defined in the spec; (c) requests referencing non-existent foreign keys with 404; (d) values exceeding spec-defined max lengths (default 500 chars for free text).",
      "DELETE SEMANTICS (mandatory): DELETE /resource/:id MUST check existence first — return 404 if not found, 204 on success. Never return 200 success for a non-existent row.",
      "ACTIVITY LOG (conditional): If plan/interfaces.md defines /api/activity or plan/workplan.json mentions activity_log, implement the activity_log table and insert one row on every POST/PUT/DELETE of other entities with {entity_type, entity_id, action: 'created'|'updated'|'deleted', actor: 'system', timestamp, summary: human-readable}. GET /api/activity returns the latest 20 rows ordered by timestamp DESC. Provide the summary string so FE can render it directly — e.g. 'Customer Acme Corp was created'.",
