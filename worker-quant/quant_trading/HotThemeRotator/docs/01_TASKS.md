...
- **P34-07 — BLOCKED on data acquisition, with three precise blockers (probe built).** `research/data_feasibility.py` + `tools/probe_t2_feasibility.py`, 15 tests; artifact `reports/research/t2_feasibility_2026-08-08.json`. Probing the real DB rather than recollection:
  1. **PIT announcement timestamp — BLOCK.** `fundamental_snapshots.available_ts` is a **fetch** timestamp, not a disclosure timestamp: **11 distinct timestamp days across 2,087 records**, median lag **280d**, lag spread **557d**. ⚠ This is a defect beyond T2 — anything that has treated `available_ts` as a PIT boundary is suspect.
  2. **Quarterly EPS depth — BLOCK.** **0 of 95 symbols** reach 5 distinct fiscal periods (max observed 3); 94 have exactly one. The "22 rows per symbol" are repeated re-fetches of one period, so a seasonal-random-walk SUE cannot be formed at all.
  3. **Ownership structure — BLOCK.** No table or column anywhere carries foreign/individual ownership share. T2's entire conditioning variable is absent.
  4. Size/liquidity controls — OK (2,774 symbols; market cap still needs shares outstanding).
  Remedies recorded per link. T2 needs **data acquisition (EDINET XBRL + 所有者別状況), not more estimator engineering**.
...
- **P35-01b consumer inventory — DONE.** `tools/audit_raw_price_consumers.py`, artifact `reports/research/raw_price_consumers_2026-08-08.json`: **35 `daily_prices` consumers, 24 SUSPECT** (returns on raw, unguarded), 3 guarded/migrated. ⚠ The suspect list includes `backtest_value_on_livelog.py` — **the producer of the E/P live-log feeding the 2026-08-26 review** — plus all four historical backtests and `api/symbol.py`. Migration order should start there. `t1_event_study_readiness.py` migrated as the reference implementation (benchmark refuses to run if its series carries an ambiguous jump; symbols with ambiguous jumps get their own skip bucket, never averaged in).
- **P35-02 event-universe price coverage — DONE for T1; full sweep pending.** Root cause confirmed in `refresh_htr_price_db.py`: `active_universe` = symbols with a bar on the LATEST date, so any name that drops out of the screen stops refreshing forever. `tools/backfill_event_universe_prices.py` extends coverage to screener ∪ event-study universe, fetching each symbol's missing tail RAW (adjustment stays in the contract, never the writer). Live run (`--t1-only`): **15/15 tickers, +2,297 bars, 0 failures** (two names got full 842-bar histories — they had no rows at all). **T1 windows: 2 → 14; matured @20D: 1 → 4; stopping rule now blocked by TIME only (96 to go at ~144/yr), no longer by data.** Full sweep completed same day: **439/439 event-universe tickers, +79,030 bars, 0 failures** — matched-control candidates and execution-report follow-ups now have price coverage too. Remaining: wire the event universe into the daily refresh so coverage stays current (one-line union in `refresh_htr_price_db.refresh`).

Post-P35 open items: repo-wide migration of the 24 suspect consumers (start with the value live-log producer); full-universe backfill + daily-refresh wiring; then the frozen waiting states (T1 sample accrual, P34-01b fills, T2 data acquisition).
