...
**Concurrency note (SQLite single-writer constraint & migration path)**  
- **Current v1 (SQLite):** SQLite is acceptable **only under a strict single-writer rule**. All writes to trading-critical tables (accounts / cash / positions / orders / fills) must be performed by **one** dedicated process (e.g., `worker-quant-writer`) to avoid lock contention (`database is locked`) and double-apply risks under at-least-once delivery.  
  - Implementation guard: acquire a Redis leader lock at startup (e.g., `SET quant:writer_lock <id> NX EX <ttl>`). If the lock cannot be acquired, the instance must run in **read-only** mode.
- **Scale-out (recommended):** migrate trading-critical state from SQLite to **Postgres** and enforce correctness with **transactions** and **row-level locks** (e.g., `SELECT ... FOR UPDATE` on the account row during cash/position updates). SQLite may remain for local cache or derived analytics, but not as the source of truth for capital safety.




**Asset lifecycle (recommended):** When archiving to MinIO, first insert a Postgres `assets` row with `status='uploading'`, then upload, then finalize to `status='ready'`. This enables safe retries and reconciliation if a crash occurs between upload and DB update.



- Brain/LLM calls should implement a circuit breaker and fallback tiers (L0/L1/L2) to handle 429/timeouts while keeping the system responsive.
