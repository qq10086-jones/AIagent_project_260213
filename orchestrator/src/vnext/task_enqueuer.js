...
export function createTaskEnqueuer({
  ...
  async function enqueueTask({ tool_name, payload, run_id, risk_level = null, idempotency_key, context }) {
    const basePayload = { ...(payload || {}), run_id };
    const idem = idempotency_key || makeIdempotencyKey(run_id, tool_name, basePayload);
    const spec = getToolSpec(tool_name);

    const regCheck = validateTaskInputAgainstRegistry({ registry, tool_name, payload: basePayload });
    if (!regCheck.ok) {
    ...
    const risk = analyzeTaskRisk(tool_name, basePayload);
    ...
    const advisory = typeof evaluatePermissionAdvisory === "function"
      ? evaluatePermissionAdvisory({ tool_name, payload: basePayload, risk: { ...risk, risk_level: finalRisk } })
      : null;
    const executionPayload = advisory
      ? { ...basePayload, permission_advisory: advisory }
      : basePayload;
