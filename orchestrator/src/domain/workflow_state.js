...
export const STEP_CONTRACTS = {
  ...
  arch_design: {
    ...
    instructions: [
      ...
      "VALIDATION CONTRACT (mandatory): For each POST/PUT endpoint, document in interfaces.md the required-field list, enum whitelists for status/priority fields, max-length constraints for text, and foreign-key existence checks (e.g. POST /api/tickets/:id/comments requires ticket :id to exist).",
  ...
  impl_fe_modules: {
    ...
    instructions: [
      ...
      "SECURITY (mandatory): Wrap EVERY user-controlled string with escapeHtml() before injecting into innerHTML or template literals. This includes: entity names, descriptions, comments, AND the message argument passed to showConfirmDialog. Bad: `Delete ${customer.name}?` Good: `Delete ${escapeHtml(customer.name)}?`. For badge class names based on status/priority, use a whitelist lookup object (const STATUS_CLASS = {open: 'badge-open', ...}), NEVER `class=\"badge ${status.toLowerCase()}\"` (CSS class injection risk).",
  ...
  impl_be: {
    ...
    instructions: [
      ...
      "Emit handoff/be_to_fe.json with api_contracts, shared_types, explicit scope_constraints, AND a 'user_controlled_fields' array listing which response fields contain user-submitted strings (e.g. ['customer.name', 'customer.email', 'ticket.title', 'ticket.description', 'comment.content']). FE uses this to know what to escape.",
      "INPUT VALIDATION (mandatory): Every POST/PUT handler MUST reject: (a) missing or blank required fields (name, title, email) with 400 + {error, field, message}; (b) status/priority values outside the enum whitelist with 400; (c) POST /api/tickets/:id/comments MUST 404 if ticket doesn't exist; (d) check max length on text fields (e.g. 500 chars for description).",
