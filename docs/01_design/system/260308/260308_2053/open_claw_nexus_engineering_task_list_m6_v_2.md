# OpenClaw Nexus
## Engineering Task List - Milestone 6 (v2)
## Date: 2026-03-08
...
| Metric | Pre-M6 State | M6 Target | Measurement Method |
|--------|--------------|-----------|-------------------|
| Real Discord workflow replay coverage | none / ad hoc | approved replay set exists and passes staging pipeline | replay manifest + staging run report |
| Step-level context budget baseline | synthetic / limited | baseline distributions collected for PM / Arch / BE / FE / QA | `metrics/baseline_context_budget.json` |
| Diff-first hit rate visibility | partially known | diff-first hit / fallback rate measurable on real replay corpus | `metrics/diff_first_baseline.json` |
| Patch anchor mismatch observability | synthetic only | mismatch rate measurable and thresholded | `metrics/patch_reliability.json` |
| FE-safe parallel eligibility visibility | limited | eligibility hit rate measurable by workflow type | `metrics/parallel_eligibility.json` |
| Parallel vs sequential outcome comparison | not established | side-by-side staging comparison exists for approved replay set | `metrics/parallel_vs_sequential.json` |
| Rollback gate operability | unproven | one-command / one-config rollback validated in staging and canary prod | rollback drill log |
| Limited production exposure | not enabled | whitelist-only production rollout completed under gate | gated production report |
...
- replay corpus is diverse enough to expose routing and budget edge cases
...
- any triggered rollback is documented rather than hidden
...
Phase 4 — Rollout governance
  WS-25-01  Production gate and rollback switches
...
- a governed replay corpus of real Discord-originated workflow patterns exists
...
- an explicit exposure go / no-go decision exists before limited production exposure
...
- approval package is complete and sufficient for milestone review
...
| Synthetic canaries hide real Discord prompt distribution edge cases | High | replay corpus with real prompt patterns is mandatory |
...
| FE-safe eligibility policy is too permissive | High | deny-by-default policy with whitelist and explicit thresholds |
...
| Rollback exists in theory but is slow in practice | High | mandatory rollback drill and runbook |
| M6 produces evidence but no clear go / no-go decision mechanism | Medium | approval and closure notes must include explicit decision branches |
