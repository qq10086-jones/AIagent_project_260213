...
def generate_execution_report(conn, run_id: str, asof: str, artifact_dir: Path,
    ...
    if len(merged) > 0:
        ...
        # Codex v3 #3: orders.qty is REMAINING (decrements on fills); using
        # it for qty_diff produces wrong (~0) diff on partial/filled rows.
        # Derive the original order qty from expected_value/limit_price
        # (invariant at place-time) with graceful fallback to
        # (remaining + filled).
        def _original_qty(row):
            ev = float(row.get("expected_value") or 0.0)
            lp = float(row.get("limit_price") or 0.0)
            if ev > 0 and lp > 0:
                return ev / lp
            return float(row.get("qty") or 0.0) + float(row.get("fill_qty") or 0.0)
        merged["original_qty"] = merged.apply(_original_qty, axis=1)
        merged["qty_diff"] = merged["fill_qty"] - merged["original_qty"]
...
def main():
    ...
    ap.add_argument("--out_dir", default="artifacts/decision",
                    help="Fallback if run snapshot_path is not set")
    ap.add_argument("--strategy_id", default=None,
                    help="Scope report to this strategy_id. Required to avoid "
                         "cross-contamination between sprint and sprint_paper "
                         "tracks (v3 codex C).")
    args = ap.parse_args()
    ...
    try:
        ...
        # Prefer strategy_id from meta if CLI omitted
        strategy_id = args.strategy_id or (meta.get("strategy_id") if meta else None)
        ...
        out_md, out_csv = generate_execution_report(
            conn, args.run_id, asof, artifact_dir, strategy_id=strategy_id,
        )
