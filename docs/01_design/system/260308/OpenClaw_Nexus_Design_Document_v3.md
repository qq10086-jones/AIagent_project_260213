...
## Design Document v3
## Date: 2026-03-08
## Supersedes: docs/01_design/system/260307/OpenClaw_Nexus_Design_Document_v2.md
...
Backend and Frontend implementation steps must output:
...
**Rationale:** Diff generation requires a known file baseline. In the Coding Team workflow sandbox, a reliable baseline is not guaranteed. Complete file output is unambiguous and worker-coder can apply it directly.

**Future roadmap (deferred from M4):** M5 will introduce structured diff / AST patching to reduce token usage and large-file truncation risk. See Section 21 (Future Roadmap).
...
| Risk ID | Risk | Severity | M4 Mitigation | Status |
|---------|------|----------|---------------|--------|
| R-1 | Context Explosion — large artifacts between steps overflow model context | High | `num_ctx` config + secondary_model fallback on overflow | Partial — context budget tracking deferred to M5 |
...
| R-4 | Pipeline Garbage Propagation — bad output in step N corrupts step N+1 | High | Handoff schema validation gates + QA two-layer validation | M4: gates added at all transitions |
...
| R-7 | Observability Gaps — LLM calls and state transitions not traceable | Medium | Structured LLM Dispatcher logs + workflow event logs + run_id traceability | M4: dispatcher logs required |
...
|------|-------------|--------|
| Structured diff / AST patching (replace full file output) | M5 | M4 priority is stability; full file output is unambiguous for now |
| BE + FE parallel execution (workflow DAG) | M5 | M4 needs stable sequential pipeline first; parallel DAG is a separate engine feature |
| LLM classification in Brain Router (Phase A upgrade) | Future | Current heuristic routing works; LLM adds non-determinism risk before pipeline is stable |
| Adaptive model routing (auto-select model by task complexity) | M7 | Requires stable baseline performance data first |
| Context budget tracking (per-step token monitoring) | M5 | R-1 partial mitigation; full solution requires context-aware chunking |
