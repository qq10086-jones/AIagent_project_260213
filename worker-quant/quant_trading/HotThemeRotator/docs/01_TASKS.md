...
**Status: BACKEND COMPLETE / FINAL ACCEPTANCE BLOCKED ON P37-02 (2026-08-12).** Governance, aggregator, CLI, log and API payload are delivered and verified; the frontend half of Rule 15.10.7 is not, so this task is deliberately NOT closed — see "Declared-but-not-yet-rendered" below. `daily_routine.ok` intentionally means the core emit/sweep collection succeeded; research diagnostics are non-fatal under Rule 16.6. Do not silently redefine that boolean. Add a separate aggregate `health_status in {healthy, degraded, failed}` and document its semantics before implementation.
...
**Declared-but-not-yet-rendered (carried into P37-02, not silently dropped).** Rule 15.10.7 binds "dashboard, CLI, and log". CLI ✅, log ✅, API payload ✅ (`meta.pipelineHealth`, verified against the real log). **Frontend rendering ❌** — the badge and its component list are not yet drawn in V1-V4. Deliberately deferred rather than duplicated: P37-02 adds a global source pill to the same shared header, and the health badge belongs in that one status strip rather than in a second parallel one. **P37-01 is not fully compliant with its own Rule 15.10.7 until P37-02 lands the strip, and its status line says so rather than claiming DONE.**
