...
function sanitizeLocalAssistantReply(raw) {
  ...
  // Safety fallback: if reply still looks like chain-of-thought, replace it.
...
async function callLocalOllamaChat(model, userInput, timeoutMs = Number(process.env.OLLAMA_CHAT_TIMEOUT_MS || 240000)) {
  ...
  const numPredict = Number(process.env.OLLAMA_NUM_PREDICT || 128);
  const systemPrompt =
    "你是 NEXUS 助手。请直接回答用户当前问题，不要重复自我介绍或通用欢迎语。不要输出思考过程。";
  const controller = new AbortController();
    ...
    const chatRes = await fetch(`${base}/api/chat`, {
      ...
      body: JSON.stringify({
        ...
        options: {
          ...
          num_predict: Number.isFinite(numPredict) ? numPredict : 128,
        },
        stream: false,
      }),
      signal: controller.signal,
  ...
  try {
    ...
    if (intent.mode_suggested === "chat" || !intent.requires_tools || intent.confidence < 0.6 || !intent.tool_name) {
      ...
      if (useCloudChat) {
        ...
        try {
          const localReply = await callLocalOllamaChat(CURRENT_LOCAL_MODEL, userInput);
          await replyChunked(
            msg,
