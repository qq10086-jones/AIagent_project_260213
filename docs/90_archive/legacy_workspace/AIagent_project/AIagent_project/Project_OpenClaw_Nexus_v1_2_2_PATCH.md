# Project OpenClaw Nexus（项目蓝图 v1.2.2）
...
4.1 Pre-Flight Check（施工前确认清单）  
5. 任务模型与状态机（Task FSM）  
6. Tool & Skill Contract（工具/技能协议 + 注册表）  
7. 大脑池（Brain Pool）与路由策略（含 Skill Routing）  
8. 安全审计官（Guard / Audit）与权限体系  
9. 数据层：事件日志与指标口径（Event & Metrics）  
10. MVP 闭环（含 Workflow Spec）  
11. 里程碑与验收标准  
...
在写第一行代码前，先把下面三类“工程地雷”一次性排掉（否则 MVP 会被胶水/环境/存储拖死）。

### 4.1.1 Redis：优先统一为 Redis Streams（跨语言一致性优先）

- **结论（v1.2.1 推荐）**：任务投递与结果回报统一走 **Redis Streams + Consumer Groups**（`XADD / XREADGROUP / XACK`），避免 Node 侧 BullMQ 与 Python 侧库生态不对齐导致的胶水成本。
- **Node（OpenClaw）侧**：用 `ioredis` 直连 Streams 即可；若未来一定要 BullMQ，也建议仅用于 OpenClaw 内部“短任务/定时任务”，不要作为跨语言主干。
- **Python（Worker）侧**：使用 `redis-py` 原生支持 Streams；Worker 以消费者组方式抢单（天然负载均衡）。

**MVP 必须写进协议的 4 件事（不写=必返工）：**
1. **ACK 语义**：Worker 完成后 `XACK`；执行中周期性写心跳（建议写到 event log/DB，而不是 Streams 本体堆日志）。
2. **可见性超时（visibility timeout）**：定义 `visibility_timeout_ms`，超过该时间仍未 ACK 的任务视为“卡死/掉线”。
3. **Pending 回收（reclaim）**：需要实现 `XAUTOCLAIM`（Redis ≥6.2）或 `XCLAIM` 来把超时 pending 任务重新分配，避免“任务永远卡在 PEL”。
4. **DLQ / 重试**：失败次数写入 payload（`retry_count`）；超过阈值写入 `stream:dlq`，并触发 Guard/人工处理。

> 推荐命名：`stream:task`, `stream:result`, `stream:dlq`；消费者组：`cg:workers`；消费者名：`{worker_name}-{pid}`。

### 4.1.2 MinIO：大对象入桶 + 生命周期（TTL）+ 去重

- **结论**：DB 只存元数据（log_id、hash、key、size、mime、ts），**截图/HTML/视频**等 evidence 与产物全部进 **MinIO**。
- **强制建议**：在 `docker-compose` 中把 MinIO 的 data 目录 **挂载到大容量盘**（最好非系统盘）。视频/素材会非常快吃满空间。
- **目录/桶分层（建议 MVP 就定）**：
  - `bucket:nexus-artifacts`：报告、CSV、产出文件（相对长期）
  - `bucket:nexus-evidence`：截图、HTML 快照、关键证据（按风险等级保留）
  - `bucket:nexus-tmp`：临时中间产物（默认 7–14 天 TTL 自动清理）

**去重与指纹（强烈建议）**
- 上传前计算 `sha256`，DB 记录 `sha256 -> object_key` 映射；同 hash 不重复存（避免爬虫 HTML/重复截图爆桶）。
- object key 推荐：`{namespace}/{yyyy}/{mm}/{dd}/{sha256[:2]}/{sha256}.{ext}`。

### 4.1.3 Guard：MVP 先做成 OpenClaw 的 Hook（但要为未来拆分留边界）

- **结论（MVP 推荐）**：Guard 不单独拆微服务，先做成 OpenClaw 的 `before_task_dispatch()` 钩子（实现快、调试简单）。
- **关键要求**：Guard 的输入/输出必须结构化，便于未来拆分为独立服务而不重写策略。

**Guard Decision Schema（建议固定）**
- `allow: bool`
- `risk_level: low | medium | high`
- `required_approval: bool`
- `policy_id: str`
- `reasons: string[]`
- `redactions?: object`（可选：对敏感字段脱敏/裁剪）
- `next?: { action: "route" | "block" | "downgrade" | "human_in_the_loop", target?: string }`

---

### 4.1.4 额外 4 条硬检查（建议写进 README 开工即执行）

1. **幂等键（idempotency_key）**：任何可产生外部副作用的动作（下单/改价/发帖/投放）必须携带幂等键；重试不得重复执行。
2. **单一事实源（SSOT）**：OpenClaw 与 Worker 的状态以 **Event Log/DB** 为准，禁止“各自维护一套状态”。
3. **降级/只读模式**：Redis 或 MinIO 不可用时，系统必须进入 **停止派发/只读**（保留查看与报告），不要“半瘫继续写”导致数据不一致。
4. **GPU 触角先服务化**：MVP 阶段把 ComfyUI/媒体生成当作外部服务依赖（特别是 Windows 环境），Docker 里只管调度与结果回传；GPU 不稳定时可降级跳过媒体步骤。
...
### 6.2 Skill Contract（在 Tool 之上：可安装、可发现、可治理的“技能”）

> **目标**：把“工具”升级为一等公民 **Skill**，像 n8n 的节点一样可组合、可版本化、可热插拔。  
> **定义**：**Skill = Tool Contract + Runtime + Capability 标签 + Packaging/治理信息**。

#### 6.2.1 Skill Manifest（建议 YAML/JSON）

每个 Skill 必须提供一个 manifest（可入库/可校验/可分发），最少字段：

- `name`, `version`, `description`
- `entrypoint`：`python:module.func | http:https://... | cli:command`
- `input_schema` / `output_schema`（沿用 Tool Contract）
- `capabilities[]`：能力标签（例：`quant.data_fetch`, `media.ffmpeg`, `commerce.listing`）
- `risk_tag`：`read_only | write_draft | write_external | money_movement`（决定是否强制人审）
- `runtime`：`in-proc | worker-proc | remote-service`
- `resources`：`gpu?`, `memory_mb`, `concurrency`
- `observability`：日志字段、metrics 名称、artifact 约定
- `packaging`：`source`（git/url），`checksum`（sha256），`deps`（镜像/requirements）

#### 6.2.2 Runtime 形态（MVP 只保留 3 种）

1) **in-proc（Node/TS）**：轻量技能（解析、路由、格式化、校验、diff）
2) **worker-proc（Python Worker）**：大多数可控执行（回测、报表、批处理）
3) **remote-service（外置服务）**：GPU/重依赖（ComfyUI、Whisper、外部 API）

> **原则**：GPU 与重依赖优先“外置服务化”，中枢只做调度与审计。

#### 6.2.3 Skill Registry（技能注册表：安装/启用/检索/禁用）

最小实现（先不做 Marketplace）：

- 表 `skills(name, version, manifest_json, checksum, source, installed_at, enabled, risk_tag)`
- CLI：`skill install <git-url>@<tag>` / `skill enable/disable` / `skill search <capability>`
- 运行时：OpenClaw 只依赖 registry 来做 **skill 发现与选择**（不把技能硬编码在代码里）
...
### 10.3 Workflow Spec（DAG：把闭环写成可执行/可视化的工作流）

> **目标**：把“工具列表”升级为 **可复用工作流**，像 n8n 一样由节点组成，但执行语义由 OpenClaw 统一掌控（SSOT + Guard）。

**最小工作流结构（YAML/JSON 均可）**
- `workflow_id`, `version`
- `trigger`：`manual | cron | webhook`
- `nodes[]`：`{id, skill, input_mapping, retry, timeout_s, risk_tag_override?}`
- `edges[]`：`{from, to}`
- `gates[]`：需要审批的节点（或由 `risk_tag` 自动推导）
- `artifacts`：节点产物的 MinIO key 规范（可被后续节点引用）

**示例（量化闭环 DAG）**
1) `fetch_market_data`（read_only）  
2) `compute_signals`（read_only）  
3) `propose_orders`（write_draft）  
4) `guard_approve_orders`（gate）  
5) `execute_orders`（money_movement）  
6) `post_trade_report`（read_only, 产出报告/图表）  
7) `update_positions`（write_external 到内部 DB，但可幂等）

> **执行语义**：节点级重试、节点级人审、节点级重跑（从任意节点重放），且全链路落 event_log。
...
### P1.5（技能生态落地：Registry + Workflow Runner）
- Skill Manifest 校验器上线（schema/风险标签/资源声明）
- Skill Registry 可安装/可禁用/可检索（capability 搜索）
- Workflow Runner 支持 DAG 执行、节点级重试/重跑、审批 Gate
