...
import { buildBackendExecutionPacket, validateBackendExecutionPacket } from "../src/coding_execution_adapters.js";
...
function main() {
  const fixturePath = path.resolve(process.cwd(), "canary_inputs", "backend_execution_adapter_min.json");
  ...
  const outDir = path.resolve(process.cwd(), "artifacts", "canary", "backend_execution_adapter");
  fs.mkdirSync(outDir, { recursive: true });
  const reportPath = path.join(outDir, "backend_execution_adapter_canary.json");
  ...
  console.log("# Backend Execution Adapter Canary");
