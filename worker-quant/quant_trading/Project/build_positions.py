...
def main():
    ...
    try:
        # base positions
        prev_d, qty, avg_cost = latest_positions(conn, args.asof)
        ...
        # simple average cost update (weighted) for BUY; for SELL we keep avg_cost
        for sym, side, q, px, fee, tax in fills:
            sym = str(sym)
            side = str(side).upper()
            q = float(q)
            px = float(px)
            total_fee = float(fee) + float(tax)
            ...
            if side == "BUY":
                new_q = cur_q + q
                if new_q > 0:
                    # weighted avg with fees included
                    new_cost_value = cur_q * cur_c + q * px + total_fee
                    avg_cost[sym] = new_cost_value / new_q
                qty[sym] = new_q
            elif side == "SELL":
                qty[sym] = cur_q - q
                # avg_cost unchanged for remaining shares
            else:
        ...
        for sym, q in sorted(qty.items()):
            px = last_close(conn, sym, args.asof)
            if px is None:
            ...
            rows_out.append((args.asof, sym, q, avg_cost.get(sym, None), px, mv, upnl))

        with conn:
            conn.execute("DELETE FROM positions WHERE asof=?", (args.asof,))
            conn.executemany(
                """
        ...
        # simple report
        print(f"✅ Built positions for {args.asof} from prev={prev_d} using run_id={args.run_id}")
