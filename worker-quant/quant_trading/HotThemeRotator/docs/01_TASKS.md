...
**Tests: `tests/unit/test_frontend_provenance_strip.py` (34).** Structural ones pin one implementation / one mount / no variant copies. The behavioural ones EXECUTE the resolvers in node against real payload shapes — including a payload generated by the live `_pipeline_health` producer, so a field rename on either side fails loudly instead of silently rendering "real" over sample data. Grep-only assertions would pass on a strip that renders nothing. Node is resolved via `shutil.which` and the tests skip cleanly when it is absent, so the daily smoke gate never acquires a JS-runtime dependency.
