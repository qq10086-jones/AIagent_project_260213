 ...
 * result_consumer.js
 *
 * Redis stream consumer for task results — drives workflow progression,
 * Discord notifications, and run lifecycle management.
...
/**
 * @param {{ pool, redis, workflowEngine, normalizeResultPayload, normalizeErrorCode,
 *           getRunInputText, findRunIdByTaskId, countPendingTasksForRun,
 *           countFailedWorkflowRunsForRun, countFailedTasksForRun,
 *           updateRunStatusIfNotFailed, updateTaskTerminalResult, forceTaskFailed,
 *           markTaskRunning, recordEvent, taskToContext, runToContext,
 *           discord, replyChunked, safeTranslate, createResultEmbed, createBinaryAttachment,
 *           insertTrace, s3, callLocalOllamaReply, detectProject,
 *           formatCodingDelegateResult, summarizeOutputBrief,
 *           deliverWorkflowRuntimeNotification,
 *           streamResult, groupResult }} deps
 */
export function createResultConsumer({
  ...
  function parseOutputField(rawOutput) {
    ...
    try { return JSON.parse(rawOutput); } catch { return { raw: String(rawOutput) }; }
  }
          ...
          for (const [id, kv] of messages) {
            ...
            if (status === "claimed") {
              ...
              const workflowTerminal = await workflowEngine
                ...
                .catch(err => { console.warn(`[workflow] handleTaskTerminal failed: ${err.message}`); return null; });

              const notifyCtx = taskToContext.get(task_id);
              await deliverWorkflowRuntimeNotification({
                workflowTerminal, notifyCtx, discord, workflowEngine, status, normalizedErrorCode,
                streamError, output: output || {}, safeTranslate, replyChunked,
              });
              ...
              if (status === "succeeded" && wfFailed) {
                const wfCode =
                  ...
                  (Array.isArray(workflowTerminal?.missing_artifacts) && workflowTerminal.missing_artifacts.length > 0
                    ? "STEP_ARTIFACT_MISSING" : "WORKFLOW_VALIDATION_FAILED");
                status = "failed";
                streamError = `Workflow validation failed: ${wfCode}`;
                const failedResult = normalizeResultPayload("failed", { ...(output || {}), workflow_validation: workflowTerminal || {} }, wfCode);
              ...
              if (ctx) {
                  ...
                  if (ctx.tool_name === "web.search_and_browse" && status === "succeeded") {
                    try {
                      ...
                      if (sentMsgs && sentMsgs.length > 0) {
                        ...
                        await insertTrace(pool, { trace_id: lastMsg.id, project_id: "general", task_type: "web_search", context_digest: originalQuestion.slice(0, 300), action_json: output || {}, metrics_json: {}, ignore_conflict: true });
                      }
                    } catch (e) {
                    ...
                    const embed = createResultEmbed({ status, title, toolName: ctx.tool_name, duration, summary: summaryText });
                    const attachments = [];
                    if (status === "succeeded" && output && Array.isArray(output.artifacts)) {
                      for (const art of output.artifacts) {
                        const isSupported = typeof art?.mime === "string" && (art.mime.startsWith("image/") || art.mime === "text/html" || art.mime === "text/markdown");
                        if (isSupported && art.object_key) {
                          ...
                          } catch (err) { console.error("S3 Download Error:", err); }
                        }
                      }
                    }
                    ...
                    try {
                      ...
                      await insertTrace(pool, { trace_id: sentMsg.id, project_id: project, task_type: ctx.tool_name, context_digest: `run_id=${ctx.run_id}`, action_json: output || {}, metrics_json: {}, ignore_conflict: true });
                    } catch (err) { console.warn("[learning] Failed to insert trace from worker result:", err.message); }
                  }
                }
                ...
                if (ctx.pendingTaskIds) {
                  ...
                  if (ctx.pendingTaskIds.size === 0 && ctx.closeRunOnTaskResult) {
                    ...
                    if (channel && typeof channel.send === "function") await channel.send(`[NEXUS] ${doneText}`);
