...
def slope_log_price(close: pd.Series, window: int = 60) -> pd.Series:
    """
    log(price) 的滚动线性回归斜率（快速版，无 sklearn）。
    使用固定的 x=0..window-1，斜率 = sum((x-x̄)*y) / sum((x-x̄)^2)，其中 y=log(price)。
    该实现为 O(n)（卷积），比 rolling.apply 快很多。
    """
    ...
    # 卷积实现 rolling dot：对齐为“窗口末端”位置
    # numer[t] = sum_{k=0..window-1} x[k] * y[t-window+1+k]
    numer = np.convolve(y, x[::-1], mode="valid")

    out = np.full_like(y, np.nan, dtype=float)
    out[int(window) - 1 :] = numer / denom

    # 若窗口内含 NaN/Inf，卷积会产生 NaN；这里再显式清理一次
...
def backtest_multi_etf_circuit_breaker(
    ...
    for i in range(train_window, len(dates) - 1):
        ...
        cost = turnover * (float(cost_bps) + float(slippage_bps)) / 10000.0 if trade_occurs else 0.0
...
if __name__ == "__main__":
    INITIAL_CAPITAL = 200_000_000  # JPY
    TICKERS = ["9432.T", "9433.T", "2914.T", "3382.T"]
    BENCHMARK = "1321.T"
    ...
    if msg == "Success":
        ...
        # =========================================================
        # 额外图表输出（新增，不改变主报告 strategy_report.html 的格式）
        # =========================================================
        try:
            bench_px_series = px[BENCHMARK].reindex(equity.index).ffill()
            bench_equity = bench_px_series / float(bench_px_series.iloc[0])
            ...
            extras = make_subplots(
                ...
                vertical_spacing=0.05,
                subplot_titles=(
                    "Equity (Strategy vs Benchmark, Relative)",
            ...
            extras.add_trace(
                go.Scatter(x=equity.index, y=equity.values, mode="lines", name="Strategy"),
                row=1, col=1
            )
            extras.add_trace(
                go.Scatter(x=bench_equity.index, y=bench_equity.values, mode="lines", name="Benchmark"),
                row=1, col=1
            )

            extras.add_trace(
                go.Scatter(x=strat_dd.index, y=(strat_dd.values * 100.0), mode="lines", name="Strategy DD"),
                row=2, col=1
            )
            extras.add_trace(
                go.Scatter(x=bench_dd.index, y=(bench_dd.values * 100.0), mode="lines", name="Benchmark DD"),
                row=2, col=1
            )

            extras.add_trace(
                go.Bar(x=stats.index, y=stats["turnover"].values, name="Turnover"),
                row=3, col=1
            )

            extras.add_trace(
                go.Bar(x=stats.index, y=(stats["cost"].values * 10000.0), name="Cost (bps)"),
                row=4, col=1
            )

            extras.update_layout(
                height=1100,
                title_text="Extra Diagnostics (kept separate; main report format unchanged)",
                template="plotly_dark",
                showlegend=True
            )

            extra_report_name = "strategy_report_extras.html"
            extras.write_html(extra_report_name)
            print(f"✅ 额外诊断图表已保存至: {extra_report_name}")

            # 额外：权重热力图（单独输出，避免改动主报告布局）
            try:
                w_heat = go.Figure(
                    data=go.Heatmap(
                        z=w_df.values.T,
                ...
                w_heat.update_layout(
                    height=500,
                    title_text="Weights Heatmap (separate output)",
                    template="plotly_dark"
                )
                weights_report_name = "weights_heatmap.html"
                w_heat.write_html(weights_report_name)
                print(f"✅ 权重热力图已保存至: {weights_report_name}")
            except Exception as _e2:
                print(f"⚠️ 权重热力图导出失败: {_e2}")

        except Exception as _e:
            print(f"⚠️ 额外图表导出失败: {_e}")
