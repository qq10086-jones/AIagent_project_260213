...
**Status: DONE (2026-08-12, closed together with P37-02).** Held at BACKEND COMPLETE until the frontend half of Rule 15.10.7 landed; P37-02's shared strip now renders the badge and every degraded component in all four variants, verified in a real browser, so the rule is satisfied end to end and the task closes. `daily_routine.ok` intentionally means the core emit/sweep collection succeeded; research diagnostics are non-fatal under Rule 16.6. Do not silently redefine that boolean. Add a separate aggregate `health_status in {healthy, degraded, failed}` and document its semantics before implementation.

**Delivered in the mandated order.** (1) Governance first: **Rule 15.10** in `docs/02_GOVERNANCE.md` — `ok` explicitly NOT redefined, the three states, stable component codes, "silence is not health", perishable labelling, exit codes 0/3/1, and the ban on publishing the aggregate without its components. (2) `src/hot_theme_rotator/observability/pipeline_health.py` — 14 declared afterclose components + 2 preopen, `assess_record()` pure and non-raising. (3) 28 contract tests. (4) `daily_routine` writes `record["health"]` and exits via `exit_code_for`. (5) `meta.pipelineHealth` on `/api/dashboard`; `enable_console_fallback()` + a real-cp932 regression for `risk_mandate_snapshot`.

**Two defects the implementation itself exposed — both found by running the aggregator against the real log rather than against fixtures.**

- **`collection` has no `ok` key.** `collect()` returns raw return codes and counts; the combined verdict (Rule 11.9's "green rc with zero new samples is not a collection, unless the day was already collected") lives in the record's top-level `ok`. The first version read a non-existent `collection["ok"]`, so **every idempotent re-run — the normal state of a re-run day — scored a CORE FAILURE**. The unit tests missed it because the fixture invented an `ok` key the producer never writes. Fixed by `_assess_collection`, which reads the return codes here and defers to `ok` there rather than re-deriving a second copy of the rule; the fixture now carries the real shape and a replay test pins the actual logged record.
- **`candidate_snapshot_present` is structurally always false.** It asks whether `asof`'s snapshot is on disk, and at 08:30 on a trading day `asof` IS today, whose snapshot is written at that day's afterclose — **false on 7 of 7 genuine pre-open runs**. Promoting it to a health signal would have shipped an alarm that fires every single morning, which is the fastest way to teach an operator to ignore health entirely. The old field is kept unchanged for existing consumers; `prior_session_snapshot_present` (is the last COMPLETED session's snapshot ready) is recorded beside it and is what health reads.

**Also fixed:** `tools/risk_mandate_snapshot.py` and `tools/daily_routine.py` were absent from `CLI_MODULES` in `tests/unit/test_cli_console_encoding.py`, so neither was ever checked; both crashed on `--help` under cp932 (U+2014, plus U+2192 in daily_routine). The missing enrolment was the defect — enrolling them is the fix, the ASCII-ification is what enrolment then demanded. A real-cp932 data-path test now covers the Japanese sleeve labels/thesis that killed a hand-run risk snapshot at the start of this session.

**Replay of the whole production log under the contract** (111 records): 57 afterclose + 40 preopen `degraded`, 12 `failed`, 2 `healthy`. Most historical `not_run` entries are components that did not exist yet in those runs — honest under replay, not current degradations. The recent window is the signal: 2026-08-10 afterclose = `degraded` on `price_db_refresh.event_universe_partial` + `tdnet_poll.nonzero_exit` (perishable), exactly the two that were invisible under `ok: true`.

**Operational note for the owner.** A degraded afterclose now exits **3**, not 0. `HTR_Daily_AfterClose` in Task Scheduler will start showing last-result 3 on days with a TDnet or event-universe degradation. That is the intended visibility, and 3 is deliberately distinct from 1 so the scheduler can be configured to treat them differently.

**Rule 15.10.7 coverage — CLOSED 2026-08-12.** The rule binds "dashboard, CLI, and log". CLI ✅, log ✅, API payload ✅ (`meta.pipelineHealth`), **frontend ✅** via P37-02's `DataProvenanceStrip`. It was deliberately deferred rather than duplicated: P37-02 needed a global source pill in the same shared header, and two parallel status headers would have been the very fragmentation both tasks exist to remove. The task was held at BACKEND COMPLETE for the interval rather than claimed DONE.

**Follow-up 2026-08-12 — the biconditional was only half implemented (found in review).** Rule 15.10.2 declares `failed` **iff** `ok is False`. The code computed `failed = (ok is False) OR (any core component looks bad)`, so `ok=True` with a missing `collection` block returned `failed` — the aggregate contradicting the very boolean it promised to agree with. The rule is right and the code was wrong: `ok` is the authority on the core verdict, so a disagreement is now REPORTED rather than resolved by overruling it. `ok=True` + a bad core component ⇒ `degraded` + `core.contract_mismatch`; a record with no `ok` at all ⇒ `degraded` + `core.ok_missing` (it cannot support a `healthy` claim); `ok=False` with nothing named ⇒ `failed` + `core.unspecified`, unchanged. The same fix applies to `daily_routine.health_block()`, whose exception fallback returned a flat `degraded` and would have understated a real core failure at exactly the moment the diagnostic meant to explain it had broken. Coverage was one-sided too — the old tests only exercised `ok=False → failed`; there is now a parametrized sweep asserting both directions across seven record shapes, plus a test that walks the **whole production log (109 non-dry-run records, 0 violations)**. No existing artifact was affected: the log never reached a contradicting shape, so this was a reachable-contract defect, not a production incident.
...
**Status: DONE (2026-08-12).** The API already emits `meta.candidatesSource` (`screener_v2 | sample`) and `meta.dataAvailability.candidates`; the frontend consumes neither `candidatesSource` string nor a source-specific label.

**Delivered — `frontend/src/htr-provenance.jsx`, one `DataProvenanceStrip` mounted ONCE in the app shell above `<main>`.** Not called from four variants: four call sites drift, one mount cannot. It answers the three fragments of a single question — is what I am looking at real and current:

- **source** from `meta.candidatesSource`, fail-closed. `real` requires a positive `=== "screener_v2"`; missing, empty, `sample`, or any future producer renders the loud `示例候选 SAMPLE — 非真实行情，不可据以下单`. Never derived from `candidates.length` (a populated array proves the mock loaded, not that the screener ran) and it does not soften because `/api/dashboard` returned 200 — a 200 that served the fixture is the case being warned about.
- **pipeline health** from `meta.pipelineHealth`, with every `degradedComponents` entry rendered, uncollapsible and untruncated (Rule 15.10.7). `unknown` — including an absent block — renders as a WARNING, never green and never silent: a missing answer and a good answer must not look the same. **Fail-closed on a missing cause (two review follow-ups).** Per-LIST: a `degraded`/`failed` badge whose component list is absent, mistyped, or a well-formed empty array synthesizes `pipeline_health.component_details_missing` rather than rendering the aggregate alone. Per-ITEM: a well-formed non-empty array can still be unusable row by row — `[null]` **crashed the render outright, which under zero-build Babel is a blank page, not a broken widget**, and `[{}]` / `["oops"]` drew a row with no stable code, the same Rule 15.10.7 failure wearing a component's clothes. Each entry must now be an object carrying a non-empty `code`; valid rows are always kept (one bad entry must not discard the real causes beside it), any unusable entry appends `pipeline_health.component_details_invalid`, and contract-error rows lead because "the display is broken" outranks any degradation listed under it. Junk surfaces even under a `healthy` badge — a malformed payload is malformed regardless of what it claims — an empty list satisfies the letter of "we rendered the list" while destroying the thing Rule 15.10.7 protects, namely that the operator can see WHAT is wrong. Downgrading the whole badge to `unknown` would also satisfy the rule but would discard the one thing the backend did report. The first version returned an empty list here **and a test pinned that as correct — the test was wrong, not just the code.** Perishable rows carry their own marker with the reason (`易逝 · 数据永久丢失，非延后重试`), because "we will catch it next run" is false for them and true for every other row in the same list.
- **section-level mock fallback**, which previously existed ONLY inside V3 — so V1/V2/V4 rendered sample markets and news with no warning at all. That inline block is deleted, not copied.

**Tests: `tests/unit/test_frontend_provenance_strip.py` (65).** Structural ones pin one implementation / one mount / no variant copies. The behavioural ones EXECUTE the resolvers in node against real payload shapes — including a payload generated by the live `_pipeline_health` producer, so a field rename on either side fails loudly instead of silently rendering "real" over sample data. Grep-only assertions would pass on a strip that renders nothing. Node is resolved via `shutil.which` and the tests skip cleanly when it is absent, so the daily smoke gate never acquires a JS-runtime dependency.

**Verified in a real browser** (uvicorn + Chrome, not only in tests — the contract tests cannot catch a JSX syntax error, which under zero-build Babel is a blank page): all four variants show exactly 1 strip / 2 component rows / 1 perishable badge; V3's old banner is gone; forcing `candidatesSource: "sample"` + a missing health block renders both loud states correctly; console clean apart from the expected in-browser-Babel notice.

**Out of scope, unchanged as declared:** the zero-build/CDN architecture stays. Removing Babel/React-development CDN would require introducing a build chain for a single-operator localhost cockpit — near-zero benefit, and `frontend_zerobuild_backup_2026-05-30/` shows the choice was deliberate.
...
- **Carried in from P37-01:** the same shared status strip must render `meta.pipelineHealth` — the `healthy | degraded | failed` badge AND every `degradedComponents` entry, with perishable ones marked. Rule 15.10.7 forbids showing the aggregate alone, and P37-01 is not compliant with its own rule until this lands. One strip, not two: source honesty and pipeline health are the same question (is what I am looking at real and current) and must not be split across two competing headers.
...
- **Step 1 - dependency layering. DONE 2026-08-13.** `src/hot_theme_rotator/observability/import_surface.py` + `tools/audit_import_surface.py` + 37 tests. The declaration is **derived from the tree, not authored**: the audit scans `src/ tools/ api/ tests/ scripts/`, tiers each importing file, and fails (exit 2) when code and declaration disagree in either direction. `tests/unit/test_import_surface.py::test_repo_import_surface_is_clean` re-runs it on every smoke run, so this cannot silently rot back.
  ...
  - **`httpx` is required and no file imports it.** 13 test files use `fastapi.testclient.TestClient`, which is built on httpx and raises at import time without it. A static scan cannot see this, so `HIDDEN_REQUIREMENTS` declares it **with a witness that the audit re-measures** — a hidden requirement whose evidence disappears is reported as stale rather than inherited forever.
  - **`numba` is not a dependency of this repo.** `tests/conftest.py` states the slow lane exists because "the only src modules importing numba/vectorbt live under `src/hot_theme_rotator/backtesting/`" — **no module imports numba at all**; it arrives transitively through vectorbt, the single importer (`backtesting/vectorbt_spike.py:56`, function-level). The lane split is still right; its stated justification was not. conftest corrected.
  - **`yfinance` is core despite every `src` import site being guarded.** The three guards return `None` on ImportError, so an install without yfinance reports "no prices" while looking healthy — the exact silent degradation Rule 15.10 forbids (P37-01). Four tools import it unguarded, three of them `daily_routine` steps. Guarded is therefore recorded as a *fact about the code*, never as a licence to call a dependency optional: `ALWAYS_REQUIRED` and `OPTIONAL_GUARDED` make each such call an explicit, reasoned entry, and an "optional" module with any unguarded site is a defect.
  - **Layering is decided by tier, not by taste.** `src/hot_theme_rotator/{research,backtesting}/**` are research tiers, so vectorbt/numpy/pandas do not enter the base install; `tools/` has **no** prefix rule because the directory mixes the daily operational lane, research backtests and a standalone UI — each tool that imports third-party code is tiered explicitly against evidence (7 of them are `daily_routine` steps), and a new untiered tool fails the audit instead of defaulting into core.
  - **Blind spot found while testing, now its own check.** A test that put a third-party import in `experiments/` expected an untiered-file defect and got **CLEAN** — the directory is not in `SCANNED_ROOTS`, so no import site was ever produced to tier. "Clean" meant "clean among the directories I was told about", the same shape as the P37-01 cp932 enrolment gap. `unscanned_source_roots` now reports any top-level directory holding `.py` files that is neither scanned nor declared an artifact root.
  - **The `.runtime` absolute-vs-relative trap was re-encountered and re-fixed.** `common/source_scan.py` now holds the single implementation of the repo walk (both `gate_reachability` and `import_surface` use it), verified a byte-for-byte no-op on gate_reachability's 423-file list before adoption. The new `find_unscanned_source_roots` promptly reproduced the same bug by matching absolute path parts — pytest's basetemp is literally named `pytest_tmp`, so every file under a temp repo looked excluded and an unscanned root reported clean. Caught by the test that had just been written for it.
  - **Review round 2 (2026-08-14) — four defects found, all in the fail-closed contract itself, all fixed before Step 1 was allowed to stand:**
    1. **The `httpx` witness was self-certifying.** It searched for the substring `"TestClient"`, and `tests/unit/test_import_surface.py` contains that word inside a synthetic fixture — so the audit counted **14 witnesses where 13 files import it**, and the test asserting httpx is required would have kept the requirement alive by its own text after every real use was deleted. The witness is now matched in the **AST** (`from fastapi.testclient import TestClient`), which counts 13 and excludes the test file. Pinned by a case whose fixture only *mentions* the words, plus a real-tree check that the audit's own test file is not among the witnesses.
    2. **The CLI could print `verdict: DEFECTS` and exit 0.** `unscanned_source_roots` — added in the same session — reached the report's verdict but not the CLI's hand-written defect counter, so a CI job consuming the exit code would have gone green on the very blind spot just closed. Fixed structurally, not locally: the report exposes one `defects` map that **both** the verdict and the CLI read, so a category cannot be added to one and forgotten by the other. Parametrized tests now drive the CLI to exit 2 for **every** category, and a completeness test fails if the category list drifts.
    3. **"`pip install .[test]` runs every test" was false in both directions,** and it was a comment asserting an install fact rather than deriving one. Now derived: `lane_requirements()` walks each pytest lane's test modules through the first-party import graph and separates **module-level** imports (needed to collect) from **deferred** ones (inside a function, may never run). Result: the **fast** lane needs `.[test,dashboard]` — the suite reaches `api/`, which imports pydantic and starlette at module level, so `.[test]` alone cannot even collect it; the **slow** lane needs `.[test,research]`. A subtlety the coarse claim missed in the other direction: a *fast*-lane test (`test_no_trade_diagnostics`) also reaches `backtesting/vectorbt_spike`, but `import vectorbt` there is inside a function while `import pandas` is module-level — so the fast lane does **not** need the research extra. `LANE_INSTALL_CONTRACT` is now checked: an uncovered module-level requirement is a defect, and hoisting that vectorbt import would fail the audit instead of breaking the fast lane on a fresh machine.
    4. **The version floors were invented.** `yfinance>=0.2.40` against 1.1.0 installed, `vectorbt>=0.26` against 1.0.0, `pytest>=8.0` against 9.0.2 — under a comment claiming they were "the version this repo is verified against". They were not, and a resolver reads `>=X` as a support claim. Every floor is now exactly the installed version; upper bounds stay absent (an untested future release is no more tested than an untested old one). `tomli>=2.0` is the one bound with no verified value and now says so: the 3.10 fallback never executes on 3.13, so 2.0 is an API floor that Step 3 will be the first to actually run.
  - **Review round 3 (2026-08-14) — two gaps in the lane machinery added by round 2, both fixed:**
    1. **The import graph dropped every relative import.** `build_import_graph` did `continue` on `node.level`, so **14 statements — 12 real edges** — were missing, every one a package `__init__` re-exporting its own submodules, which is precisely the edge a reachability question travels along. Now resolved to absolute names (`from .x import y` in a package `__init__` resolves against the package; in a plain module against the parent; climbing above the top is dropped, never guessed), with 6 synthetic regression cases plus a real-tree check. **The P34-00 governance conclusion was re-derived on the corrected graph and is unchanged:** `min_entry_score` and `min_leader_score` are still `dormant` with zero entrypoint paths, so the O-2 downgrade that rested on it still stands. That is recorded as a test, not assumed — the graph was wrong and the answer held anyway, which is the least reassuring way for a graph to be wrong. A second limit found while fixing it is now stated rather than left implicit: parent-package execution is not modelled at all (`import a.b.c` runs `a/__init__.py`, but only `a.b.c` becomes an edge), which under-counts in the direction that makes `dormant` harder to disprove.
    2. **The slow lane's `research` requirement was protected by nothing.** vectorbt is the entire reason the slow lane exists, but its import is inside a function, so the collection-floor check — which only looks at module-level imports — never saw it: removing `research` from the slow contract left the audit reporting CLEAN. `DEFERRED_RUNTIME_REQUIREMENTS` now names it explicitly (module + function + reason), and the audit checks both that the lane's contract covers it and that the named function still imports it, so a stale declaration is reported instead of believed.
  - **Wording corrected accordingly.** "Lane contracts are derived from the import graph" was too strong. Precisely: the **collection floor** is statically checked (a lane cannot even collect without it); the **runtime requirement** is a declared, witnessed entry that static analysis cannot prove, because whether a deferred import executes depends on whether anything calls it. Only step 3's clean-environment install and run turns the second half into a verified fact.
  - **Not claimed:** no version constraint here is verified, and **no install has been performed** — the lane contracts are derived from the import graph, not from a clean-environment install. Lower bounds are the versions this repo has run against, not minimum-working versions; `requires-python = ">=3.10"` is supported by a syntax scan (no 3.11+ stdlib feature is used anywhere) but the only interpreter ever used is CPython 3.13.0 on Windows. Step 3 owns all of that verification.
  - **Deferred, measured, not fixed:** **45 tools define `main()` and are not enrolled in `tests/unit/test_cli_console_encoding.py`; 22 of them have a `--help` first line a cp932 console cannot encode.** P37-01 concluded "the list itself was the gap" and enrolled 2. The new tool is enrolled; the other 45 are a P37-01 follow-up, not P37-03 scope.
  - **Verification: full fast smoke 2577 passed / 1 skipped / 0 failed** (2514 + 38 for step 1, + 15 for review round 2, + 10 for round 3). The skip is unrelated to this change and is reported rather than rounded up to a pass: `test_frontend_provenance_strip.py:422` requires a perishable degraded component in the *real* log and today's has none (2026-08-12's had `tdnet_poll.nonzero_exit`). That contract assertion did not execute today — the cost of replaying real payloads instead of fixtures, and the honest reading is "not run", not "passed".
- **Step 2 — reproducible lock mechanism. DONE 2026-08-14.** `tools/compile_locks.py` + `requirements/{runtime,fast,slow,dev}.txt` + `requirements/verified-environment.txt` + `requirements/README.md` + 23 tests. Locks are compiled with `uv pip compile --generate-hashes`; every entry carries sha256 hashes so `pip install --require-hashes` is meaningful.
  - **The lock reproduces the environment the suite passed in, not the newest satisfying set.** Those are different answers: an unconstrained resolve of this project today picks `yfinance 1.6.0` and `beautifulsoup4 4.15.0`, **neither of which has ever run here** — locking that would be the same invented compatibility step 1 removed from the version bounds, with more decimal places. Every resolution is constrained by `verified-environment.txt`, and the tool **exits 2 if any pin differs from it**. Current result: runtime 29 / fast 48 / slow 88 / dev 110 pins, **zero drift**.
  - ⚠ **A wrong lock was nearly generated, and the cause is worth keeping.** `uv` resolves against **its own managed interpreter** by default — on this machine a CPython **3.12.11** under `AppData` holding huggingface/transformers, which reports `requests==2.32.5` where the interpreter that runs pytest has **2.34.2**. The first constrained compile failed with "requests==2.32.5 unsatisfiable", which is what exposed it; taken at face value the fix would have looked like relaxing the bound, and the lock would have pinned a machine nobody runs. The snapshot is therefore taken with `python -m pip freeze` from the **running** interpreter, and every compile passes an explicit `--python-version 3.13 --python-platform x86_64-pc-windows-msvc`. A test asserts the tool keeps naming both.
  - **One lock per install contract, and the lane names are checked against `LANE_INSTALL_CONTRACT`** so step 1's derived contracts and step 2's artifacts cannot drift apart: `runtime` (base only, the daily operational lane) / `fast` (`.[test,dashboard]`) / `slow` (`.[test,research]`) / `dev` (all extras). Tests assert each lane's collection floor **and** its declared runtime requirement are pinned in its own lock, that the four locks never disagree on a shared package, and that every pin carries a hash.
  - **Two step-1 conclusions independently confirmed by the resolver, not by the same static scan that produced them:** `numba` appears **only** in `slow.txt` at 0.65.1, pulled in by vectorbt and declared by nobody; and `pydantic`/`starlette` really are in `fast.txt`, because the fast lane reaches `api/`. `vectorbt`, `numba` and `llvmlite` are **absent from `fast.txt`** — which is what makes the deferred import in `vectorbt_spike.py` load-bearing rather than stylistic, and is pinned by a test.
  - **Not claimed:** **no install has been performed from any lock.** They resolve, their pins equal the verified environment, and they cover the derived contracts — that is a *verified resolution*, not a *verified install*. The locks are also **CPython 3.13 / x86_64 Windows only**: hashes pin wheels and wheels are per-platform, so `requires-python = ">=3.10"` remains unlocked and untested. Both belong to step 3.
- **Step 3 — clean-environment install, import, startup and run. DONE 2026-08-14.** `tools/verify_clean_environments.py` builds one venv per lane under `.runtime/P37-03/envs/`, installs from the lock with `--require-hashes`, and runs the lane. It refuses anything but CPython 3.13 x64 Windows, pins TMP/TEMP/TMPDIR and `PYTHONNOUSERSITE=1` for every child process, and will only delete paths under its own `envs/` directory. Evidence artifacts: `reports/engineering/clean_env/{fast,slow}_2026-08-14.json` (gitignored; the numbers below are the recorded run).
  - **The build toolchain was the hole, and it was real.** Measured: a fresh CPython 3.13 venv contains **`pip 24.2` and nothing else** — ensurepip stopped shipping setuptools in 3.12. So `pip install .` has build isolation fetch an **unlocked setuptools from PyPI**, straight past every hash, and `pip install --no-build-isolation .` fails outright. `requirements/bootstrap.in` → `bootstrap.txt` now locks pip + setuptools with hashes; it is installed first and the project is built `--no-deps --no-build-isolation --no-index`, so nothing is fetched outside a lock. A related gap fell out: plain `pip freeze` **omits pip/setuptools/wheel**, so the first bootstrap lock resolved pip unconstrained to **26.2.1** where this machine runs **25.3** — the build toolchain would have been the one unpinned part of a hash-pinned install. The snapshot now uses `pip freeze --all`.
  - **runtime lane** — 30 packages. Project installed from the build, and `import hot_theme_rotator` resolves to `envs/runtime/Lib/site-packages/...`, i.e. the installed distribution rather than `src/`. 5 daily-lane modules import; yfinance 1.1.0 present; `assess_record` self-check returns `degraded` with no external service contacted. `pytest`, `fastapi`, `vectorbt`, `numba`, `streamlit`, `httpx` all **absent**, as the lock intends.
  - **fast lane** — 50 packages. `create_app()` succeeds (33 routes); `TestClient` `/api/health` → **200 `{"status": "ok"}`**; a **real uvicorn subprocess** on a loopback free port answered `/api/health` with 200, then **terminated with no survivor and the port released**. ⚠ That is *not* a graceful shutdown and is no longer described as one: graceful stop was attempted and does not work from a separate process on Windows — with `CREATE_NEW_PROCESS_GROUP`, `CTRL_BREAK_EVENT` exits **3** (`STATUS_CONTROL_C_EXIT`, i.e. killed rather than handled) and `CTRL_C_EVENT` does not stop the server within 20s at all. The probe therefore asserts termination, absence of a survivor and a released port, and deliberately asserts no exit code, since the code would only encode how it was killed. `pytest -m "not slow"` → **2666 passed, 1 skipped, 19 deselected in 81.13s**, run by the lane's own interpreter — the reading *at step 3*, before steps 4-6 added their own tests. The current figure is in the step 7 table and is larger for that reason, not because the lane changed behaviour. `vectorbt`, `numba`, `llvmlite` **provably absent** and the lane still passes — which is what makes the deferred import in `vectorbt_spike.py` load-bearing rather than stylistic.
  - **`requires-python` narrowed `>=3.10` → `>=3.13`, on evidence.** The old floor rested on a syntax scan (PEP 604/585 generics used, no 3.11+ stdlib feature anywhere). Sound reasoning, untested claim. Only CPython 3.13.0 on x86_64 Windows has ever run this code; the locks are hash-pinned to cp313 wheels and both CI lanes assert 3.13. `>=3.10` was worse than declaring nothing, because a resolver reads it as support and would install onto an interpreter the project has never seen — the same error as the invented version floors from review round 2, one level up. Widen it again behind a verified matrix, never behind a scan. Consequence: tomli's `python_version < "3.11"` marker is now unselectable, so it never installs and never appears in a lock; the declaration stays because the path it names is a fail-closed refusal, and the lock test asserts its absence as a consequence rather than an oversight.
  - **The verification polluted the tree it verifies, and the audit caught it.** Building in place leaves `build/lib/**` — **172 generated copies of `src/*.py`** — plus `src/*.egg-info`. The import-surface audit immediately reported `build` as an unscanned source root, correctly: neither scanned nor excused. Both are now declared artifact directories and gitignored.
  - **Not claimed:** nothing here verifies any other platform or interpreter, and `api/`/`tools/` remain unpackaged — the dashboard and CLIs run from the checkout, which is the shipped topology and is what the fast lane was verified in.
- **Step 4 — fast/slow classification audit. DONE 2026-08-14.** `observability/test_classification.py` + `tools/audit_test_classification.py` + 15 tests. Two independent angles, neither of them a filename.
  - **The measured angle found what the declared angle could not.** `test_wild_cluster_bootstrap_size_is_not_liberal_on_the_real_shape` was taking **92.92 seconds in the fast lane — half its entire 184s wall time**. Its arguments are `n_sims=300` and `n_boot=199`: **both under the threshold individually**, ~60,000 regressions when nested. Reading arguments one at a time is how a scan can be thorough and still miss the most expensive test in the suite. The scanner now takes the **product** of a call's scale arguments, which catches it statically from here on.
  - **The durations also indicted this milestone's own work.** Eleven tests each re-ran a full import-surface audit at ~2.2s — about **29s added to the daily gate by the step meant to protect it**. They are contract tests that belong in the fast lane, so they now share one module-scoped report; tests that monkeypatch module state deliberately keep their own uncached audit, since a shared report would answer for the unmodified tree and pass without testing anything.
  - **Result: fast lane 184.27s → 75.52s local / 81.13s in the clean environment**, with research-scale tests moved to slow (**14 function-level `slow` markers** after the sklearn cross-check joined them in step 3), each carrying its measured or declared reason at the marker. Small fixed-seed known-answer checks stay in the fast lane on purpose; losing them from the daily gate would be a real loss of coverage.
- **Step 5 — Windows TMP/TEMP/cache/basetemp. DONE 2026-08-14.** `common/runtime_paths.py` + 18 tests. Four scratch roots had accumulated at the repo top level (`pytest_tmp`, `pytest_cache`, `.pytest_tmp`, `.pytest_cache`) because three callers each invented their own; there was no owner. More importantly **none of them set TMP/TEMP**, so `--basetemp` moved pytest's own scratch while every library calling `tempfile` stayed in the system temp — the directory whose ACL defect produces this project's known false hangs and mass ERRORs.
  - ⚠ **A second defect surfaced while testing the fix, and it is now a regression test: pytest does NOT create missing parents for `--basetemp`.** Pointing it at an uncreated path **errors every `tmp_path` test** (18 of 22 in the reproduction) — the same signature as the ACL failure. So a "fix" that pins the path without creating it reproduces the exact symptom it was introduced to remove. `lane_paths` creates eagerly and refuses to fall back to the system temp; `daily_routine` now creates explicitly before running rather than relying on a side effect.
...
- **Step 6 — two independent CI jobs. DONE 2026-08-14 (structurally verified, never executed).** The repo had no `.github/` at all, so this is a first CI rather than a reconfiguration: `.github/workflows/fast-smoke.yml` and `slow-research.yml`, both `windows-latest` + CPython 3.13 x64. **Two files, not two steps in one job** — chained in one shell the last exit code decides, and a research regression would ride out on a fast-lane success (Rule 15.6). Each installs the locked bootstrap toolchain, then its own lock, every install `--require-hashes`, then the project `--no-deps --no-build-isolation --no-index`; asserts the interpreter it actually got rather than trusting `setup-python`; pins TMP/TEMP/TMPDIR and pytest scratch into the workspace `.runtime`, failing closed if the directories cannot be created; sets `PYTHONNOUSERSITE=1`; uses no secrets; takes read-only permissions; and ends by asserting it left the tree unmodified. Each proves what the other cannot — fast asserts `vectorbt`/`numba`/`llvmlite` are **absent**, slow asserts they are present **and that calling the grid really executes the deferred import**. `tests/unit/test_ci_workflows.py` holds **36** structural assertions over the parsed YAML. (It held 30 when the workflows were first written, and those 30 all passed against files GitHub could never have found — see the deployment error below. The count below is the historical one on purpose.)
  - ⚠ **DEPLOYMENT ERROR, found in review and fixed 2026-08-14: the workflows were in a place GitHub Actions never looks.** Both were committed under `worker-quant/quant_trading/HotThemeRotator/.github/workflows/`. This project is a **subdirectory** of the repository (`git rev-parse --show-toplevel` → `E:/AIagent_project_260213`), and Actions reads only `<repo root>/.github/workflows/`. Pushed as they were, **neither workflow would ever have been discovered or run** — a CI that exists as text and not as a check.
    - **The 30 structural tests passed against them,** because `test_ci_workflows.py` resolved the directory as `PROJECT_ROOT/.github/workflows`. The test derived its location from the thing under test, so it could only ever confirm it. This is the same failure shape as the `httpx` self-certifying witness: evidence that is generated by the subject cannot test the subject.
    - **Fixed:** both files moved to the repository root with `git mv` (joining, never replacing, the pre-existing `validate-registry.yml`); the nested `.github/` removed; location now derived from `git rev-parse --show-toplevel` and asserted against **the path git actually records** (`git ls-files --full-name`), plus a test that no workflow hides inside the project directory and one that the pre-existing root workflow survives. Also fixed while there: `pull_request` watched only the project directory, so **a PR that edited nothing but a workflow would not have run it** — the workflow's own path is now on both triggers. 36 structural tests.
...
- **Step 7 — end-to-end acceptance. DONE 2026-08-14.** Every claim below is from a run made after the last code change, on this machine.
  ...
  | test classification audit (static) | **CLEAN**, 54 scale sites, **14** marked slow |
  | lock offline consistency + P37-03 focused tests | **157 passed** |
  ...
  | clean env, fast lock | **2672 passed / 1 skipped / 19 deselected / 81.70s**; `create_app()`, TestClient 200, real uvicorn 200, terminated with no survivor and the port released |
  | clean env, slow lock | **19 passed / 2667 deselected / 124.18s**; deferred vectorbt import proven to execute |
  | working env, full fast | **2672 passed / 1 skipped / 19 deselected / 80.33s** |
  | working env, full slow | **19 passed / 2673 deselected / 123.85s** |
  | `git diff --check` | clean |
  | `git status` scope | **clean for HotThemeRotator + the two root workflow files only** — the wider repository has many unrelated changes in neighbouring projects, untouched by this milestone |
...
- **P36-10 — items 2–4 executed; a NEW identification threat found (2026-08-10, draft v5). Still NOT frozen.** `research/full_model_power.py`, 18 tests (2 marked `slow` — 121 day dummies on 420 rows is a 23-minute run, research lane not smoke lane).
  ...
  - **Power of the ACTUAL primary specification** (disjoint window, full model, event-day FE, ρ=0.3), verified: size 0.152 without day FE → **0.033 with**; power at β₁ = 0.10/0.20/0.30/0.50 → **0.10/0.23/0.40/0.77**. ⚠ **The remedy costs power and the number is stated, not omitted**: at β₁=0.30 power falls ~0.55 → **0.40**. Any eventual result must be read against 0.40.
...
- **P36-11 — v5's "items 2-4 complete" WITHDRAWN; three defects found in review and fixed (2026-08-10, draft v6). NOT frozen.** All three reproduced before acceptance.
  1. **The joint Holm simulation was a no-op.** It took an overlap RATIO and added `overlap x sigma x shared` to both outcomes - a constant, absorbed entirely by the intercept. Reproduced: overlap 0.0 / 0.382 / 1.0 gave IDENTICAL power to three decimals, so "159 shared events jointly simulated" was false. Fixed: the join report now emits `bucket_events` (419 per bucket, 159 shared; singleton days H1 52 / H2 60) and the simulator draws ONCE per unique event (679), so shared events carry the same realised values into both regressions. Verified to matter: real overlap both=0.175 vs artificially disjoint both=0.025.
  2. **FY and event-day fixed effects are exactly collinear** - an event day lies in one fiscal year, but v5 assigned FY per EVENT, which cannot happen in data and hid it. Real structure: 420x126, rank 124, cond 4e16, and **np.linalg.inv does not raise** - it returned slope 0.6354 against a truth of 0.30 (0.2575 with the redundant block dropped). Fixed: FY per day, FY dummies dropped under day FE, and `ols_cluster_robust` checks rank before inverting and refuses. Two regression tests pin both properties.
