...
- **P36-01 — DONE 2026-08-09: T2's last missing input acquired; chain now `feasible: True`.** `data/external/edinet_ownership.py` + `tools/backfill_edinet_ownership.py`, 16 tests. Element IDs confirmed against a **live** filing (doc S100YNWZ, 4750.T), not assumed from taxonomy docs. **Live backfill: 3,988 stored / 4,000 attempted — 3,810 symbols, 0 errors** (7 documents carry no ownership block, 5 failed validation), published 2025-06-27..2026-07-02.
  - **Two traps handled:** (a) the XBRL values are **FRACTIONS despite a 「（％）」 label** (68.83% → `0.6883`) — `validate_ownership()` requires the categories to partition to ~1.0 and names the percent/fraction confusion when a sum lands near 100, because a 100×-wrong conditioning variable would look entirely plausible; (b) ownership is an **INSTANT at fiscal year end, public at submitDateTime** ⇒ annual PIT snapshot with a validity window; consumers join on `published_ts`, never `as_of`.
  - **The cohort exists and is large:** foreign ownership median 5.84% (p10 0.54%, p90 30.20%), individual median 46.28%. **Low-foreign (<5%) ∧ high-individual (>50%) = 1,215 symbols** — the segment Jinushi reports drift still lives in.
  - **Two further self-corrections in the same round:** the probe now MEASURES ownership coverage (a 12-symbol smoke test read as "ready" — same premature-victory class as the withdrawn 439/439 claim; ≥500 symbols now required), and `FeasibilityReport.feasible` now requires every required link to be fully `available`, matching the module's own documented contract (`degraded` surfaces separately via `not_ready`).
  - **T2 is now a research-design task, not a data task.** Remaining before any outcome read: freeze a preregistration (event definition, SUE construction at ANNUAL frequency, conditioning buckets, horizons, benchmark), register the trial family, then accumulate. Note the frequency caveat above — quarterly SUE would need 四半期報告書.
